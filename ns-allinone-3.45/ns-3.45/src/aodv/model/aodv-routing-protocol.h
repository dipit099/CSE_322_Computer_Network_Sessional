...
namespace ns3
{
    ...
    namespace aodv
    {
        ...
        private:
            ...
            // ============ CC-AODV + ECC-AODV Modifications Variables ============

            // --- CC-AODV Base ---
            uint32_t m_congestionCounter; ///< CC-AODV: Congestion counter per node
            uint32_t m_baseThreshold;     ///< CC-AODV: Base threshold for RREQ dropping

            // --- Modification 1: Adaptive Threshold Management (ATM) ---
            bool m_enableATM; ///< Enable ATM (adaptive threshold)

            // --- Modification 2: Queue-Aware Congestion Detection (QACD) ---
            bool m_enableQACD;        ///< Enable QACD
            double m_w1;              ///< QACD weight for queue occupancy
            double m_w2;              ///< QACD weight for counter ratio
            double m_w3;              ///< QACD weight for drop rate
            uint32_t m_totalDrops;    ///< Total RREQ packets dropped (for drop rate)
            uint32_t m_totalForwards; ///< Total RREQ packets forwarded (for drop rate)
            double m_congestionLevel; ///< Computed QACD congestion level (0.0 to 1.0)

            // --- ATM Methods ---
            uint32_t CalculateAdaptiveThreshold(); ///< ATM: Compute adaptive threshold
            bool IsNodeCongested();                ///< Decision: should this RREQ be dropped?

            // --- QACD Methods ---
            double CalculateCongestionLevel(); ///< QACD: Compute congestion level
                                               // ============================================================
