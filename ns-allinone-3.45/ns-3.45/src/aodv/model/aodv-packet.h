    ...
    namespace aodv
    {
        ...
        */
        ...
        public:
            ...
            // ============ CC-AODV / ECC-AODV: Congestion fields in RREP ============

            /**
             * @brief Set the congestion flag (CC-AODV / ATM)
             * @param flag 0 or 1
             */
            void SetCongestionFlag(uint8_t flag)
            {
                m_congestionFlag = flag;
            }

            /**
             * @brief Get the congestion flag (CC-AODV / ATM)
             * @return 0 or 1
             */
            uint8_t GetCongestionFlag() const
            {
                return m_congestionFlag;
            }

            // --- Modification 3: Optimized Packet Header Design (OPHD) ---

            /**
             * @brief Set congestion level (2-bit: 0-3)
             * 00=No, 01=Low, 10=Moderate, 11=High
             * @param level 0-3
             */
            void SetCongestionLevel(uint8_t level)
            {
                m_eccInfo = (m_eccInfo & 0x3F) | ((level & 0x03) << 6);
            }

            /**
             * @brief Get congestion level (2-bit: 0-3)
             * @return 0-3
             */
            uint8_t GetCongestionLevel() const
            {
                return (m_eccInfo >> 6) & 0x03;
            }

            /**
             * @brief Set hop quality metric (4-bit: 0-15)
             * Encodes average queue occupancy along path (0=empty, 15=full)
             * @param quality 0-15
             */
            void SetHopQuality(uint8_t quality)
            {
                m_eccInfo = (m_eccInfo & 0xC3) | ((quality & 0x0F) << 2);
            }

            /**
             * @brief Get hop quality metric (4-bit: 0-15)
             * @return 0-15
             */
            uint8_t GetHopQuality() const
            {
                return (m_eccInfo >> 2) & 0x0F;
            }
        ...
        private:
            ...
            uint8_t m_congestionFlag; ///< CC-AODV / ATM: 1-byte congestion flag
            uint8_t m_eccInfo;        ///< OPHD: packed byte [CL:2 | HopQuality:4 | reserved:2]
        };
