    ...
    namespace aodv
    {
        ...
        RoutingProtocol::RoutingProtocol()
            : m_rreqRetries(2),
              ...
              m_baseThreshold(4),     // CC-AODV: threshold = 4 (base paper)
              m_enableATM(false),     // Modification 1: ATM disabled by default (set by simulation)
              m_enableQACD(false),    // Modification 2: QACD disabled by default
              m_w1(0.4),              // QACD: queue occupancy weight
              m_w2(0.4),              // QACD: counter ratio weight
              m_w3(0.2),              // QACD: drop rate weight
              m_totalDrops(0),        // QACD: total RREQ drops
              m_totalForwards(0),     // QACD: total RREQ forwards
              m_congestionLevel(0.0), // QACD: computed CL
        ...
        RoutingProtocol::GetTypeId()
        {
            static TypeId tid =
                TypeId("ns3::aodv::RoutingProtocol")
                    ...
                    // ============ ECC-AODV Attributes ============
                    // Modification 1: Adaptive Threshold Management (ATM)
                    .AddAttribute("EnableATM",
                                  "ECC-AODV Mod-1: Enable Adaptive Threshold Management. "
                                  "Threshold adapts based on congestion counter and queue size. "
                                  "When disabled, uses fixed threshold (CC-AODV base behavior).",
                                  BooleanValue(true),
                                  MakeBooleanAccessor(&RoutingProtocol::m_enableATM),
                                  MakeBooleanChecker())
                    .AddAttribute("BaseThreshold",
                                  "ECC-AODV: Base threshold for congestion control. "
                                  "When congestion counter exceeds threshold, RREQs are dropped.",
                                  UintegerValue(4),
                                  MakeUintegerAccessor(&RoutingProtocol::m_baseThreshold),
                                  MakeUintegerChecker<uint32_t>())

                    // Modification 2: Queue-Aware Congestion Detection (QACD)
                    .AddAttribute("EnableQACD",
                                  "ECC-AODV Mod-2: Enable Queue-Aware Congestion Detection. "
                                  "Uses multi-metric CL formula instead of simple counter comparison.",
                                  BooleanValue(false),
                                  MakeBooleanAccessor(&RoutingProtocol::m_enableQACD),
                                  MakeBooleanChecker())
                    .AddAttribute("QACDWeightQueue",
                                  "QACD weight for queue occupancy (w1).",
                                  DoubleValue(0.4),
                                  MakeDoubleAccessor(&RoutingProtocol::m_w1),
                                  MakeDoubleChecker<double>(0.0, 1.0))
                    .AddAttribute("QACDWeightCounter",
                                  "QACD weight for counter/threshold ratio (w2).",
                                  DoubleValue(0.4),
                                  MakeDoubleAccessor(&RoutingProtocol::m_w2),
                                  MakeDoubleChecker<double>(0.0, 1.0))
                    .AddAttribute("QACDWeightDropRate",
                                  "QACD weight for packet drop rate (w3).",
                                  DoubleValue(0.2),
                                  MakeDoubleAccessor(&RoutingProtocol::m_w3),
                                  MakeDoubleChecker<double>(0.0, 1.0))

                    // ============ End ECC-AODV Attributes ============
        ...
        RoutingProtocol::RecvRequest(Ptr<Packet> p, Ipv4Address receiver, Ipv4Address src)
        {
            ...
            // ======== CC-AODV + ATM + QACD: Congestion Control Check ========
            if (IsNodeCongested())
            {
                m_totalDrops++; // QACD: track drops for drop rate
                NS_LOG_DEBUG("CC-AODV/ATM/QACD: Dropping RREQ due to congestion "
                             "(counter="
                             << m_congestionCounter << ", threshold=" << CalculateAdaptiveThreshold()
                             << ", CL=" << m_congestionLevel << ")");
                return;
            }
            m_totalForwards++; // QACD: track forwards for drop rate
            // ============================================================
        ...
        RoutingProtocol::SendReply(const RreqHeader &rreqHeader, const RoutingTableEntry &toOrigin)
        {
            ...
            // ============ CC-AODV / ECC-AODV: Set congestion info in RREP ============
            // Always set flag=1 so intermediate nodes increment their congestion counter
            rrepHeader.SetCongestionFlag(1);

            // Modification 2 (QACD): Set congestion level in RREP header
            if (m_enableQACD)
            {
                double cl = CalculateCongestionLevel();
                uint8_t clLevel = 0; // 00 = No congestion
                if (cl >= 0.7)
                    clLevel = 3; // 11 = High congestion
                else if (cl >= 0.5)
                    clLevel = 2; // 10 = Moderate congestion
                else if (cl >= 0.3)
                    clLevel = 1; // 01 = Low congestion

                rrepHeader.SetCongestionLevel(clLevel);

                // Modification 3 (OPHD): Set hop quality (queue occupancy 0-15)
                uint32_t qSize = m_queue.GetSize();
                uint32_t qMax = m_queue.GetMaxQueueLen();
                uint8_t hopQuality = (qMax > 0) ? (uint8_t)(15.0 * qSize / qMax) : 0;
                rrepHeader.SetHopQuality(hopQuality);

                NS_LOG_DEBUG("ECC-AODV SendReply: CL=" << cl << " level=" << (int)clLevel
                                                       << " hopQuality=" << (int)hopQuality);
            }
            // ===================================================================
        ...
        RoutingProtocol::RecvReply(Ptr<Packet> p, Ipv4Address receiver, Ipv4Address sender)
        {
            ...
            // ======= CC-AODV: Increment congestion counter on RREP

            if (rrepHeader.GetCongestionFlag() == 1)
            {
                m_congestionCounter++;
                NS_LOG_DEBUG("ECC-AODV: RREP with congestion flag received, counter incremented to "
                             << m_congestionCounter);
            }
            // ==============================================================================
        ...
        RoutingProtocol::SendRerrWhenBreaksLinkToNextHop(Ipv4Address nextHop)
        {
            ...
            // ============ ECC-AODV: Decrement congestion counter on route error ============
            if (m_congestionCounter > 0)
            {
                m_congestionCounter--;
                NS_LOG_DEBUG("ECC-AODV: Route error, congestion counter decremented to "
                             << m_congestionCounter);
            }
            // ================================================================================
        }
        ...
        // ===== ECC-AODV Modification 1: Adaptive Threshold Management (ATM) =====
        uint32_t
        RoutingProtocol::CalculateAdaptiveThreshold()
        {
            if (!m_enableATM)
            {
                // CC-AODV base: fixed threshold
                return m_baseThreshold;
            }

            // ECC-AODV ATM: Adaptive Threshold Management
            // Formula: threshold = base_threshold + (congestion_counter / 2) + (queue_size / 4)
            //
            // Key insight: When congestion increases, we RAISE the threshold.
            // This prevents over-aggressive RREQ dropping that fragments the network.
            // Higher threshold = more tolerance before dropping RREQs.

            uint32_t threshold = m_baseThreshold;

            // Factor 1: Congestion counter contribution
            // Higher congestion counter -> higher threshold (more tolerance)
            uint32_t congestionBonus = m_congestionCounter / 2;
            threshold += congestionBonus;

            // Factor 2: Queue size contribution
            // Higher queue occupancy -> higher threshold (more tolerance to prevent cascading drops)
            uint32_t queueBonus = m_queue.GetSize() / 4;
            threshold += queueBonus;

            // Cap at 3 * base threshold to prevent unbounded growth
            uint32_t maxThreshold = m_baseThreshold * 3;
            if (threshold > maxThreshold)
            {
                threshold = maxThreshold;
            }

            NS_LOG_DEBUG("ECC-AODV ATM: base=" << m_baseThreshold << " congBonus=" << congestionBonus
                                               << " queueBonus=" << queueBonus << " final=" << threshold);

            return threshold;
        }

        // ===== CC-AODV + ATM: Congestion decision =====
        bool
        RoutingProtocol::IsNodeCongested()
        {
            if (m_enableQACD)
            {
                // QACD: Use congestion level threshold
                m_congestionLevel = CalculateCongestionLevel();
                bool congested = m_congestionLevel >= 0.7; // High congestion threshold
                NS_LOG_DEBUG("ECC-AODV QACD IsNodeCongested: CL=" << m_congestionLevel
                                                                  << " congested=" << congested);
                return congested;
            }

            // CC-AODV / ATM: Simple counter-based decision
            uint32_t currentThreshold = CalculateAdaptiveThreshold();
            bool congested = m_congestionCounter > currentThreshold;

            NS_LOG_DEBUG("CC-AODV/ATM IsNodeCongested:" << " counter=" << m_congestionCounter
        ...
        // ===== Modification 2: Queue-Aware Congestion Detection (QACD) =====
        double
        RoutingProtocol::CalculateCongestionLevel()
        {
            // CL = w1 * (Qcurrent/Qmax) + w2 * (counter/threshold) + w3 * dropRate
            //
            // w1=0.4, w2=0.4, w3=0.2 (configurable via attributes)
            // Values clamped to [0.0, 1.0]

            // Term 1: Queue occupancy
            uint32_t qSize = m_queue.GetSize();
            uint32_t qMax = m_queue.GetMaxQueueLen();
            double queueOccupancy = (qMax > 0) ? ((double)qSize / qMax) : 0.0;

            // Term 2: Counter ratio (how close to threshold)
            uint32_t threshold = CalculateAdaptiveThreshold();
            double counterRatio = (threshold > 0) ? ((double)m_congestionCounter / threshold) : 0.0;
            if (counterRatio > 1.0)
                counterRatio = 1.0;

            // Term 3: Drop rate (fraction of RREQs dropped)
            uint32_t total = m_totalDrops + m_totalForwards;
            double dropRate = (total > 0) ? ((double)m_totalDrops / total) : 0.0;

            double cl = m_w1 * queueOccupancy + m_w2 * counterRatio + m_w3 * dropRate;

            NS_LOG_DEBUG("QACD CL: qOcc=" << queueOccupancy << " cRatio=" << counterRatio
                                          << " dropRate=" << dropRate << " CL=" << cl);
