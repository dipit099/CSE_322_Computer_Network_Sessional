...
RoutingProtocol::GetTypeId()
{
    static TypeId tid =
        TypeId("ns3::aodv::RoutingProtocol")
            ...
            // CHANGE for the command terminal needed during BUILDing and running projec
            .AddAttribute("EnableCcAodv",
                          "Indicates whether Congestion Control AODV is enabled.",
                          BooleanValue(false),
                          MakeBooleanAccessor(&RoutingProtocol::m_enableCcAodv),
                          MakeBooleanChecker())
            .AddAttribute("BaseThreshold",
                          "Base threshold for congestion control in CC-AODV.",
                          UintegerValue(4),
                          MakeUintegerAccessor(&RoutingProtocol::m_baseThreshold),
                          MakeUintegerChecker<uint32_t>())
            .AddAttribute("EnableEccAodv",
                          "Indicates whether Enhanced Congestion Control AODV is enabled.",
                          BooleanValue(false),
                          MakeBooleanAccessor(&RoutingProtocol::m_enableEccAodv),
                          MakeBooleanChecker())
            ...
            .AddAttribute("QACDWeightDropRate",
                          "QACD w3: weight for drop-rate term.",
                          DoubleValue(0.2),
...
RoutingProtocol::SendReply(const RreqHeader& rreqHeader, const RoutingTableEntry& toOrigin)
{
    ...
    {
        // Both CC-AODV and ECC-AODV need the flag for counter feedback
    ...
    if (m_enableEccAodv)
    {
        ...
        else if (m_congestionLevel >= 0.5)
        {
            clLevel = 2;
        }
        else if (m_congestionLevel >= 0.3)
        {
            clLevel = 1;
        }
...
RoutingProtocol::RecvReply(Ptr<Packet> p, Ipv4Address receiver, Ipv4Address sender)
{
    ...
    double newScore = 0.0;
    bool eccBetterScore = false;
    if (m_enableEccAodv)
    {
        newScore = computeRrepScore(rrepHeader);
        auto bestIt = m_bestRrepScore.find(dst);
        if (bestIt == m_bestRrepScore.end() || newScore > bestIt->second)
        {
            eccBetterScore = true;
    ...
    if (m_routingTable.LookupRoute(dst, toDst))
    {
        ...
        if (m_enableEccAodv)
        {
            // ECC-AODV: Prioritize Multi-Metric Path Selection (MMPS) scoring
            // ONLY use ECC scoring for route selection, ignore AODV conditions
        ...
        {
            // Vanilla AODV: Use standard AODV route update conditions
